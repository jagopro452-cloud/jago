...
class BusinessInfoController extends BaseController
{
    ...
    public function store(BusinessInfoStoreOrUpdateRequest $request)
    {
        ...
        if ($activationMode && $activationMode->value == 1 && $martBaseUrl && $martBaseUrl->value != null) {
            $name = businessConfig('business_name', BUSINESS_INFORMATION)?->value ?? "JAGO";
            ...
            try {
                $response = Http::post($martBaseUrl->value . '/api/v1/configurations/store', [
                    'jago_business_name' => $name,
                    'jago_business_logo' => $logo,
                    'jago_app_url_android' => $cta?->value && $cta?->value['play_store']['user_download_link'] ? $cta?->value['play_store']['user_download_link'] : "",
                    'jago_app_url_ios' => $cta?->value && $cta?->value['app_store']['user_download_link'] ? $cta?->value['app_store']['user_download_link'] : "",
                ]);
