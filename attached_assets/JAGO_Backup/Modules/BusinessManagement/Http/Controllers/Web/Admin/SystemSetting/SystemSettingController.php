...
class SystemSettingController extends BaseController
{
    ...
    public function envUpdate(Request $request): Renderable|RedirectResponse
    {
        ...
        try {
            ...
            self::setEnvironmentValue('APP_DEBUG', $request['app_debug'] ?? config('app.debug'));
            self::setEnvironmentValue('APP_MODE', $request['app_mode'] ?? config('app.mode'));
        } catch (\Exception $exception) {
    ...
    public function dbIndex(): Renderable
    {
        ...
        $tables = collect($tables)->pluck('tablename')->toArray();
