                                ...
                                <div class="row gy-4">
                                    ...
                                    <div class="col-lg-4">
                                        ...
                                        <div class="vehicle-category-zone">
                                            @php
                                                $groupedVc = $vehicleCategories->groupBy('type');
                                                $vcTypeLabels = ['car' => 'Car', 'motor_bike' => 'Bike', 'auto' => 'Auto'];
                                                $vcTypeColors = ['car' => 'primary', 'motor_bike' => 'success', 'auto' => 'warning'];
                                            @endphp
                                            @forelse($vcTypeLabels as $vcType => $vcLabel)
                                                @if(isset($groupedVc[$vcType]))
                                                    <div class="mb-1">
                                                        <span class="badge badge-{{ $vcTypeColors[$vcType] ?? 'secondary' }} mb-1">{{ translate($vcLabel) }}</span>
                                                        @foreach($groupedVc[$vcType] as $vc)
