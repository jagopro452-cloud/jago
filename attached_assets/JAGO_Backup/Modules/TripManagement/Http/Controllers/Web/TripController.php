...
class TripController extends BaseController
{
    ...
    public function tripList(?Request $request, ?string $type = null)
    {
        ...
        #vehicle category type filter (car/bike/auto)
        if ($request->has('vehicle_category_type')) {
            if ($request->vehicle_category_type && $request->vehicle_category_type != ALL) {
                $attributes['vehicle_category_type'] = $request->vehicle_category_type;
            }
        }
        ...
        $trips = $this->tripRequestService->index(criteria: $attributes, relations: ['tripStatus', 'customer', 'driver', 'fee', 'safetyAlerts', 'vehicleCategory'], orderBy: ['created_at' => 'desc'], limit: paginationLimit(), offset: $request['page'] ?? 1, appends: $request->all());
        $trip_counts = null;
        if ($type == 'all') {
