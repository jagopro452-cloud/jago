...
class DriverService extends BaseService implements Interfaces\DriverServiceInterface
{
    ...
    private function overview($driver)
    {
        ...
        $totalMorningTime = $driver->timeLog()
            ...
            ->whereRaw("online >= '06:00:00' AND offline <= '11:59:59'")
            ->sum('online_time');
        ...
        $totalMiddayTime = $driver->timeLog()
            ->whereDate('created_at', Carbon::today())
            ->whereRaw("online >= '12:00:00' AND offline <= '15:59:59'")
            ->sum('online_time');

        // Calculate Evening time
        $totalEveningTime = $driver->timeLog()
            ->whereDate('created_at', Carbon::today())
            ->whereRaw("online >= '16:00:00' AND offline <= '20:59:59'")
            ->sum('online_time');
        ...
        $totalNightTime = $driver->timeLog()
            ...
            ->where(function ($query) {
                $query->whereRaw("online >= '21:00:00' AND offline <= '23:59:59'");
            })
            ->orWhere(function ($query) {
                $query->whereRaw("online >= '00:00:00' AND offline <= '05:59:59'");
            })
            ->sum('online_time');
