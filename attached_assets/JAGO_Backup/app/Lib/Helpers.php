...
if (!function_exists('businessConfig')) {
    ...
    {
        try {
            $cacheKey = 'biz_cfg_' . $key . '_' . ($settingsType ?? 'all');
            return Cache::remember($cacheKey, now()->addMinutes(30), function () use ($key, $settingsType) {
...
if (!function_exists('checkExternalConfiguration')) {
    function checkExternalConfiguration(string $externalBaseUrl, int|string $externalToken, int|string $jagoToken): bool
    {
        ...
        return $activationMode == 1 && $martBaseUrl == $externalBaseUrl && $martToken == $externalToken && $systemSelfToken == $jagoToken;
    }
}
...
if (!function_exists('get_cache')) {
    function get_cache(string $key, int $ttlMinutes = 30): array|object|string|null
    {
        return Cache::remember($key, now()->addMinutes($ttlMinutes), function () use ($key) {
            return businessConfig($key)?->value;
        });
    }
}
if (!function_exists('get_cache_short')) {
    function get_cache_short(string $key, int $ttlMinutes = 5): array|object|string|null
    {
        return Cache::remember('short_' . $key, now()->addMinutes($ttlMinutes), function () use ($key) {
            return businessConfig($key)?->value;
        });
    }
}
...
if (!function_exists('getNotification')) {
    ...
    {
        $cacheKey = 'notification_' . $key . '_' . ($group ?? '') . '_' . ($type ?? '');
        $notification = Cache::remember($cacheKey, now()->addMinutes(30), function () use ($key, $group, $type) {
            return FirebasePushNotification::query()
                ->where('name', $key)
...
if (!function_exists('maxUploadSize'))
{
    function maxUploadSize(string $fileType): int
    {
        ...
        $appLimit = config('app.mode', 'live') === 'demo' ? convertToBytes('1M') : convertToBytes($fileType === 'image' ? '20M' : '50M');
