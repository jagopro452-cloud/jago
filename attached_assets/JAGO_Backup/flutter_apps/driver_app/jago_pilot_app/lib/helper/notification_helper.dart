...
import 'package:jago_pilot_app/features/chat/screens/message_screen.dart';
...
import 'package:jago_pilot_app/features/notification/widgets/receipt_confirmation_bottomsheet.dart';
import 'package:jago_pilot_app/features/profile/screens/edit_profile_screen.dart';
import 'package:jago_pilot_app/features/profile/screens/profile_screen.dart';
import 'package:jago_pilot_app/features/profile/widgets/level_congratulations_dialog_widget.dart';
import 'package:jago_pilot_app/features/refer_and_earn/controllers/refer_and_earn_controller.dart';
import 'package:jago_pilot_app/features/refer_and_earn/screens/refer_and_earn_screen.dart';
import 'package:jago_pilot_app/features/review/screens/review_screen.dart';
import 'package:jago_pilot_app/features/ride/screens/ride_request_list_screen.dart';
import 'package:jago_pilot_app/features/safety_setup/controllers/safety_alert_controller.dart';
import 'package:jago_pilot_app/features/trip/screens/trip_details_screen.dart';
...
import 'package:jago_pilot_app/features/trip/screens/payment_received_screen.dart';
import 'package:jago_pilot_app/features/trip/screens/review_this_customer_screen.dart';
...
class NotificationHelper {
  ...
  static Future<void> showNotification(RemoteMessage message, FlutterLocalNotificationsPlugin fln, bool data) async {
    ...
    bool isParcel = message.data['action'] == 'new_parcel_request' ||
        message.data['type'] == 'parcel' ||
        (message.data['action'] ?? '').toString().contains('parcel');

    try {
      await showBigPictureNotificationHiddenLargeIcon(title, body, orderID, image, fln, isParcel: isParcel);
    } catch (e) {
      await showBigPictureNotificationHiddenLargeIcon(title, body, orderID, null, fln, isParcel: isParcel);
  ...
  static Future<void> showTextNotification(String title, String body, String orderID, String action, FlutterLocalNotificationsPlugin fln) async {
    ...
    await fln.show(title.hashCode, title, body, platformChannelSpecifics, payload: action);
  }
  ...
  static Future<void> showBigPictureNotificationHiddenLargeIcon(String title, String body, String? orderID, String? image, FlutterLocalNotificationsPlugin fln, {bool isParcel = false}) async {
    ...
    final String channelId = isParcel ? 'jago_parcel' : 'jago';
    final String channelName = isParcel ? 'JAGO Parcel' : 'jago';
    final String soundName = isParcel ? 'parcel_notification' : 'notification';
    ...
    await fln.show(title.hashCode, title, body, platformChannelSpecifics, payload: orderID);
  }
 ...
 void _whenNewRequestFound(RemoteMessage message){
   Get.find<RideController>().ongoingTripList().then((value){
     if((Get.find<RideController>().ongoingTrip ?? []).isEmpty){
       ...
       bool isParcelRequest = message.data['action'] == 'new_parcel_request' || message.data['type'] == 'parcel';
       audio.play(AssetSource(isParcelRequest ? 'parcel_notification.wav' : 'notification.wav'));
