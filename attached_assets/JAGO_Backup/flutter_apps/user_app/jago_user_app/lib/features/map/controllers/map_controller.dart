...
class MapController extends GetxController implements GetxService {
  ...
  LatLng? _lastOwnPosition;
  double _lastOwnBearing = 0;
  Timer? _ownAnimationTimer;
  ...
  void updateMarkerAndCircle({LatLng? latLng}) async {
    ...
    if(Get.find<RideController>().tripDetails != null && _polylineCoordinateList.isNotEmpty) {
      LatLng targetPosition = latLng ?? _polylineCoordinateList.first;
      double targetBearing = _calculateBearing(
        _polylineCoordinateList.first,
        _polylineCoordinateList.length > 1 ? _polylineCoordinateList[1] : _polylineCoordinateList.last,
      );

      if (_lastOwnPosition != null) {
        _ownAnimationTimer?.cancel();
        final LatLng startPos = _lastOwnPosition!;
        final double startBearing = _lastOwnBearing;
        int step = 0;

        _ownAnimationTimer = Timer.periodic(
          ...
          (timer) {
            ...
            if (step >= _animationSteps) {
              timer.cancel();
              _placeOwnMarker(targetPosition, targetBearing, car);
            } else {
              ...
              _placeOwnMarker(interpolated, interpolatedBearing, car);
            }
          },
        );
      } else {
      ...
      _lastOwnPosition = targetPosition;
      _lastOwnBearing = targetBearing;
    }
  }

  void _placeOwnMarker(LatLng position, double bearing, Uint8List icon) {
    markers.removeWhere((marker) => marker.markerId.value == "my_location");
    markers.add(Marker(
  ...
  void setOwnCurrentLocation(LatLng? latLng, {double? heading}) async{
    markers.removeWhere((marker) => marker.markerId.value == "my_location");

    Uint8List myIcon = await convertAssetToUnit8List(Images.mapLocationIcon, width: 100);

    if(latLng != null){
      double actualBearing = heading ?? _lastOwnBearing;

      markers.add( Marker(
          markerId: const MarkerId("my_location"),
          position: latLng,
          rotation: actualBearing,
          draggable: false,
          zIndexInt: 2,
          flat: true,
          anchor: const Offset(0.5, 0.5),
      ...
      _lastOwnPosition = latLng;
      _lastOwnBearing = actualBearing;
  ...
  Future<void> getDriverToPickupOrDestinationPolyline(String lines,{bool mapBound = false}) async {
    if(lines.isNotEmpty) {
      ...
      if (result.isNotEmpty) {
        ...
        _addPolyLine(polylineCoordinates);
        _polylineCoordinateList = polylineCoordinates;
        _animateDriverMarker(_polylineCoordinateList);
  ...
  void _animateDriverMarker(List<LatLng> latLngList) async {
    if (Get.find<RideController>().tripDetails == null || latLngList.isEmpty) return;
    ...
    LatLng targetPosition = latLngList.first;
    double targetBearing = _calculateBearing(
      latLngList.first,
  ...
  void updateDriverMarker(List<LatLng> latLngList) async{
    _animateDriverMarker(latLngList);
  }
