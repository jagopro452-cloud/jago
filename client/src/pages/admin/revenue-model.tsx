...
type ModuleConfig = {
  moduleName: string;
  revenueModel: "free" | "commission" | "subscription" | "hybrid";
...
function ModuleRow({ mod, onSaved }: { mod: ModuleConfig; onSaved: () => void }) {
  ...
  const isFree = form.revenueModel === "free";
  const isSubscription = form.revenueModel === "subscription";
  const eff = isFree || isSubscription ? 0 : form.commissionPercentage;
  const gst = isFree || isSubscription ? 0 : form.commissionGstPercentage;
  const exFare = 200;
  const commAmt = exFare * (eff / 100);
  const gstAmt = commAmt * (gst / 100);
  const driverGet = isSubscription ? exFare : exFare - commAmt - gstAmt;
    ...
    <tr style={{ verticalAlign: "middle" }}>
      ...
      <td>
        {editing ? (
          ...
          <span className={`badge rounded-pill ${form.revenueModel === "free" ? "bg-dark" : form.revenueModel === "commission" ? "bg-primary" : form.revenueModel === "subscription" ? "bg-success" : "bg-warning text-dark"}`}
            style={{ fontSize: 11 }}>
      ...
      <td>
        {editing && !isSubscription ? (
          ...
          <span className="text-muted" style={{ fontSize: 12 }}>Plan based</span>
        ) : (
          <span className="fw-semibold" style={{ fontSize: 13, color: isFree ? "#111827" : "#1a73e8" }}>{eff}%</span>
        )}
      </td>
      <td>
        {editing && !isSubscription ? (
          ...
          <span className="text-muted" style={{ fontSize: 12 }}>Inside plan</span>
        ) : (
...
export default function RevenueModelPage() {
  ...
  const modules: ModuleConfig[] = (data?.modules || []).map((m: any) => ({
    ...
    revenueModel: m.revenueModel || "free",
    commissionPercentage: parseFloat(m.commissionPercentage) || 0,
    commissionGstPercentage: parseFloat(m.commissionGstPercentage) || 0,
    ...
    <div className="p-4">
      ...
      <div className="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="mb-0 text-muted" style={{ fontSize: 13 }}>Launch free now, then enable commission, subscription, or hybrid whenever needed</p>
        </div>
      </div>
      ...
      <div className="row g-3 mb-4">
        {[
          ...
          { label: "Free Modules", value: modules.filter(m => m.revenueModel === "free").length, icon: "bi-stars", color: "#111827", bg: "#F3F4F6" },
          ...
          { label: "Hybrid Modules", value: modules.filter(m => m.revenueModel === "hybrid").length, icon: "bi-layers-fill", color: "#d97706", bg: "#fef3c7" },
          { label: "Avg Commission Rate", value: `${avgComm}%`, icon: "bi-graph-up-arrow", color: "#0f766e", bg: "#ecfeff" },
        ].map((s, i) => (
        ...
        <div className="card-body p-0">
          <div className="d-flex align-items-center justify-content-between px-4 py-3"
            ...
            <div>
              ...
              <p className="mb-0 text-muted" style={{ fontSize: 12 }}>Each service can stay free today and be switched to commission, subscription, or hybrid later</p>
            </div>
          </div>
          {isLoading ? (
      ...
      <div className="mt-4 p-3 rounded-3 d-flex gap-3" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0" }}>
        ...
        <div style={{ fontSize: 13 }}>
          ...
          If <em>free</em>, the trip runs with no platform deduction.
          If <em>commission</em> or <em>hybrid</em>, the configured % is deducted from the fare and credited to admin.
          If <em>subscription</em>, the driver must have an active subscription plan when you decide to enable that mode.
