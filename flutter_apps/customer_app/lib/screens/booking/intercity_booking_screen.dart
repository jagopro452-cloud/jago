...
class _IntercityBookingScreenState extends State<IntercityBookingScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final cities = routes.map((r) => r['from_city'] as String).toSet().toList();
        cities.sort();
        if (mounted) setState(() {
          _routes = routes;
          _fromCities = cities;
  ...
  void _onFromCityChanged(String? city) {
    setState(() {
      _fromCity = city;
      _selectedRoute = null;
      _toRoutes = _routes.where((r) => r['from_city'] == city).toList();
    });
  }

  double _calculateFare(Map<String, dynamic> route) {
    final base = double.tryParse(route['base_fare']?.toString() ?? '0') ?? 0;
    final km = double.tryParse(route['estimated_km']?.toString() ?? '0') ?? 0;
    final perKm = double.tryParse(route['fare_per_km']?.toString() ?? '0') ?? 0;
    final toll = double.tryParse(route['toll_charges']?.toString() ?? '0') ?? 0;
    ...
    try {
      ...
      final res = await http.post(
        ...
        body: jsonEncode({
          ...
          'pickupAddress': _selectedRoute!['from_city'],
          'destinationAddress': _selectedRoute!['to_city'],
        }),
      );
      final data = jsonDecode(res.body);
        ...
        Container(
          ...
          child: DropdownButtonFormField<Map<String, dynamic>>(
            ...
            items: _toRoutes.map((r) {
              final fare = _calculateFare(r as Map<String, dynamic>);
  ...
  Widget _routeInfoCard() => Container(
    ...
    child: Column(children: [
      Row(children: [
        Expanded(child: _routeStat(Icons.my_location_outlined, 'From', _selectedRoute!['from_city'] ?? '')),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 8),
          child: const Icon(Icons.arrow_forward, color: _blue),
        ),
        Expanded(child: _routeStat(Icons.location_on_outlined, 'To', _selectedRoute!['to_city'] ?? '')),
      ]),
      const Divider(height: 20),
      Row(children: [
        _miniStat('Distance', '${_selectedRoute!['estimated_km'] ?? 0} km'),
        _miniStat('Toll', '₹${_selectedRoute!['toll_charges'] ?? 0}'),
        _miniStat('Total Fare', '₹${_calculateFare(_selectedRoute!).toStringAsFixed(0)}'),
      ]),
    ]),
  );
  ...
  Widget _buildBookBar() {
    final fare = _calculateFare(_selectedRoute!);
    return Container(
      ...
      child: Row(children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('$_passengers passenger${_passengers > 1 ? 's' : ''} · ${_selectedRoute!['from_city']} → ${_selectedRoute!['to_city']}',
