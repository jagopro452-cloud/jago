...
class _ParcelBookingScreenState extends State<ParcelBookingScreen> {
  ...
  Future<void> _bookParcel() async {
    if (_receiverNameCtrl.text.trim().isEmpty) {
      _showSnack('Receiver name enter cheyyandi');
      return;
    }
    if (_receiverPhoneCtrl.text.length < 10) {
      _showSnack('Valid 10-digit phone number enter cheyyandi');
      return;
    }
    if (_destAddress.isEmpty && _destCtrl.text.isEmpty) {
      _showSnack('Delivery address enter cheyyandi');
      return;
    }
    ...
    try {
      ...
      final notes = '📦 Parcel | Vehicle: ${_selectedVehicle?['name'] ?? ''} | Category: ${_selectedCategoryIcon ?? ''}${_selectedCategoryName ?? 'Other'} | Weight: ${_selectedWeightLabel ?? 'Light'} | Receiver: ${_receiverNameCtrl.text.trim()} (+91 ${_receiverPhoneCtrl.text}) | ${_instructionsCtrl.text.isNotEmpty ? 'Note: ${_instructionsCtrl.text}' : ''}';
      final res = await http.post(
