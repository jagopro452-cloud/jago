...
class _TripsHistoryScreenState extends State<TripsHistoryScreen> {
  ...
  Future<void> _showReceipt(BuildContext ctx, String tripId) async {
    showDialog(context: ctx, barrierDismissible: false,
      builder: (_) => const Center(child: CircularProgressIndicator(color: Color(0xFF1E6DE5))));
    try {
      final token = await AuthService.getToken();
        ...
        child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFF1E6DE5).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)),
              child: const Text('PAID', style: TextStyle(color: Color(0xFF1E6DE5), fontWeight: FontWeight.bold, fontSize: 12)),
            ),
          ]),
          const SizedBox(height: 16),
          const Divider(),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('₹${fare['payable'] ?? fare['total'] ?? 0}',
              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Color(0xFF1E6DE5))),
          ]),
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Container(
                          width: 44, height: 44,
                          decoration: BoxDecoration(
                            color: (isCompleted ? const Color(0xFF1E6DE5) : isCancelled ? Colors.red : Colors.orange).withValues(alpha: 0.1),
                            borderRadius: BorderRadius.circular(12)),
                          child: Icon(
                            isCompleted ? Icons.check_circle_outline : isCancelled ? Icons.cancel_outlined : Icons.directions_bike,
                            color: isCompleted ? const Color(0xFF1E6DE5) : isCancelled ? Colors.red : Colors.orange,
                            size: 24)),
                        ...
                        Row(children: [
                          Expanded(
                            child: OutlinedButton.icon(
                              ...
                              icon: const Icon(Icons.receipt_long, size: 15, color: Color(0xFF1E6DE5)),
                              label: const Text('Receipt', style: TextStyle(fontSize: 12, color: Color(0xFF1E6DE5), fontWeight: FontWeight.w600)),
                          ...
                          if ((t['destinationAddress'] ?? '').isNotEmpty) Expanded(
                            child: OutlinedButton.icon(
                              ...
                              icon: const Icon(Icons.refresh_rounded, size: 15, color: Color(0xFF1E6DE5)),
                              label: const Text('Book Again', style: TextStyle(fontSize: 12, color: Color(0xFF1E6DE5), fontWeight: FontWeight.w600)),
                              style: OutlinedButton.styleFrom(
                                side: const BorderSide(color: Color(0xFFBFDBFE)),
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                                padding: const EdgeInsets.symmetric(vertical: 8)),
