...
function commissionLabel(fr: any) {
  if (fr.commission_type === "flat") return `₹${Number(fr.commission_flat || 0).toFixed(0)}/ride`;
  return `${Number(fr.commission_percent || 0).toFixed(1)}%`;
}
...
type FormState = {
  ...
  zoneId: string; commissionType: "percentage" | "flat";
  commissionPercent: string; commissionFlat: string; isActive: boolean;
};

const emptyForm: FormState = {
  name: "", ownerName: "", email: "", password: "", phone: "",
  zoneId: "", commissionType: "percentage", commissionPercent: "", commissionFlat: "", isActive: true,
};
...
export default function FranchiseesPage() {
  ...
  const openEdit = (fr: any) => {
    ...
    setForm({
      ...
      phone: fr.phone || "", zoneId: fr.zone_id || "",
  ...
  const handleSave = () => {
    saveMutation.mutate({
      ...
      password: form.password || undefined, phone: form.phone,
    ...
    <div className="p-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <small className="text-muted">Zone-wise franchise partners — admin sets commission, franchisee earns it</small>
        </div>
      ...
      <div className="card border-0 shadow-sm" style={{ borderRadius: 12 }}>
        <div className="card-body p-0">
          {isLoading ? (
            ...
            <div className="text-center py-5 text-muted"><i className="bi bi-building fs-1 d-block mb-2"></i>No franchises yet.</div>
          ) : (
                ...
                <tbody>
                  {franchisees.map(fr => (
                    <tr key={fr.id}>
                      ...
                      <td>
                        {fr.zone_name
                          ? <span className="badge rounded-pill" style={{ background: "#7c3aed18", color: "#7c3aed" }}><i className="bi bi-map me-1"></i>{fr.zone_name}</span>
                          : <span className="text-muted small">No zone</span>}
                      </td>
                      <td>
                        {/* Commission type pill — clear what admin set */}
                        <div className="d-flex align-items-center gap-2">
                          <span className="badge rounded-pill fw-semibold"
                            ...
                            {fr.commission_type === "flat"
                              ? <><i className="bi bi-currency-rupee me-1"></i>₹{Number(fr.commission_flat || 0).toFixed(0)} / ride</>
                              : <><i className="bi bi-percent me-1"></i>{Number(fr.commission_percent || 0).toFixed(1)}% of fare</>
                            }
                          </span>
                        </div>
                        <div className="text-muted mt-1" style={{ fontSize: 11 }}>
                          On ₹200 ride → <strong style={{ color: "#16a34a" }}>₹{commissionPreview(fr, 200).toFixed(0)}</strong> to franchise
                        </div>
                      </td>
            ...
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: 16 }}>
              ...
              <div className="modal-body px-4 py-3">
                <div className="row g-3">
                  ...
                  {/* Commission section — clearly labeled */}
                  <div className="col-12">
                      ...
                      {form.commissionType === "percentage" ? (
                        <div className="d-flex align-items-center gap-2">
                          <div className="input-group" style={{ width: 180 }}>
                            <input type="number" className="form-control fw-semibold" placeholder="0"
                              value={form.commissionPercent} onChange={e => f("commissionPercent", e.target.value)}
                              min={0} max={100} step={0.5} style={{ fontSize: 18 }} />
                        ...
                        <div className="d-flex align-items-center gap-2">
                          <div className="input-group" style={{ width: 180 }}>
                            ...
                            <input type="number" className="form-control fw-semibold" placeholder="0"
                              value={form.commissionFlat} onChange={e => f("commissionFlat", e.target.value)}
                              min={0} step={0.5} style={{ fontSize: 18 }} />
                          </div>
                      ...
                      {previewComm > 0 && (
                        <div className="mt-3 rounded-2 p-2 d-flex align-items-center gap-2"
                          ...
                          <span style={{ fontSize: 13 }}>
                            On a <strong>₹{previewFare}</strong> ride →&nbsp;
                            franchise earns <strong style={{ color: "#16a34a", fontSize: 15 }}>₹{previewComm.toFixed(2)}</strong>
                            {form.commissionType === "percentage" && <span className="text-muted"> ({form.commissionPercent}% of fare)</span>}
                            {form.commissionType === "flat" && <span className="text-muted"> (fixed amount)</span>}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="modal-header border-0 px-4 pt-4 pb-0">
                <div>
                  ...
                  <div className="d-flex align-items-center gap-2 mt-1">
                    {viewingStats.zone_name && <span className="badge rounded-pill" style={{ background: "#7c3aed18", color: "#7c3aed" }}><i className="bi bi-map me-1"></i>{viewingStats.zone_name}</span>}
                    <span className="badge rounded-pill fw-semibold"
                      style={{
                    ...
                    <div className="table-responsive" style={{ maxHeight: 250, overflowY: "auto" }}>
                      <table className="table table-sm table-hover mb-0">
                        <thead style={{ background: "#f8f9fb", position: "sticky", top: 0 }}>
                          <tr><th>Ref</th><th>Customer</th><th>Fare</th><th>Franchise Earned</th><th>Status</th></tr>
                        </thead>
                        <tbody>
                          {(stats.recentTrips || []).map((t: any) => (
                            <tr key={t.ref_id}>
                              ...
                              <td className="small fw-semibold" style={{ color: "#16a34a" }}>
                                ₹{commissionPreview(viewingStats, Number(t.total_fare || 0)).toFixed(0)}
                              </td>
                              <td>
                                <span className={`badge rounded-pill ${t.current_status === "completed" ? "bg-success" : t.current_status === "cancelled" ? "bg-danger" : "bg-secondary"}`} style={{ fontSize: 10 }}>
                                  {t.current_status}
                                </span>
                              </td>
                            </tr>
                          ))}
                          {!(stats.recentTrips || []).length && <tr><td colSpan={5} className="text-center text-muted py-3">No trips yet</td></tr>}
                        </tbody>
                      </table>
                    </div>
                  </>
