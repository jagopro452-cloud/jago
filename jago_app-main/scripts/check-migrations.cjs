...
const UNSAFE_PATTERNS = [
  { re: /^\s*CREATE INDEX\s+(?!(?:CONCURRENTLY\s+)?IF NOT EXISTS)/im, label: "CREATE INDEX without IF NOT EXISTS" },
  { re: /^\s*CREATE UNIQUE INDEX\s+(?!(?:CONCURRENTLY\s+)?IF NOT EXISTS)/im, label: "CREATE UNIQUE INDEX without IF NOT EXISTS" },
