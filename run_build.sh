...
setup_flutter_git() {
  local PREV_DIR
  PREV_DIR=$(pwd)
  echo "[SETUP] Setting up Flutter git repo (force-clean)..."
  cd "$FLUTTER_HOME"
  # Always force-recreate to guarantee clean state — prevents "not a clone" errors
  rm -rf .git
  git init 2>/dev/null
  git config user.email "flutter@flutter.dev"
  git config user.name "Flutter"
  git checkout -b stable 2>/dev/null || git checkout -b main 2>/dev/null
  ...
  git commit --allow-empty -m "Flutter 3.27.4"
  local REAL_HASH
  ...
  git -C "$FLUTTER_HOME" remote get-url origin 2>/dev/null || echo "[WARN] Remote not set"
}

git config --global user.email "build@jago.app" 2>/dev/null || true
git config --global user.name "JAGO Build" 2>/dev/null || true

# Always recreate Flutter git to guarantee clean state on every build
setup_flutter_git
...
# Re-init flutter git right before build — flutter config/pub may have dirtied git state
setup_flutter_git
fix_flutter_version
...
if [ $CUSTOMER_EXIT -eq 0 ]; then
  cp build/app/outputs/flutter-apk/app-release.apk /home/runner/workspace/JAGO-Customer-v1.0.26.apk
  SIZE=$(ls -lh /home/runner/workspace/JAGO-Customer-v1.0.26.apk | awk '{print $5}')
  echo ""
  echo "=========================================="
  echo "SUCCESS: JAGO Customer APK → $SIZE"
  echo "   File: JAGO-Customer-v1.0.26.apk"
  echo "=========================================="
else
...
# Re-init flutter git right before build
setup_flutter_git
fix_flutter_version
flutter build apk --release 2>&1
...
if [ $DRIVER_EXIT -eq 0 ]; then
  cp build/app/outputs/flutter-apk/app-release.apk /home/runner/workspace/JAGO-Pilot-v1.0.26.apk
  SIZE=$(ls -lh /home/runner/workspace/JAGO-Pilot-v1.0.26.apk | awk '{print $5}')
  echo ""
  echo "=========================================="
  echo "SUCCESS: JAGO Pilot APK → $SIZE"
  echo "   File: JAGO-Pilot-v1.0.26.apk"
  echo "=========================================="
else
...
set +e
GH_TOKEN="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"
GH_REPO="jagopro452-cloud/jago"
VERSION="v1.0.26"
CUSTOMER_APK="/home/runner/workspace/JAGO-Customer-v1.0.26.apk"
DRIVER_APK="/home/runner/workspace/JAGO-Pilot-v1.0.26.apk"

if [ -f "$CUSTOMER_APK" ] || [ -f "$DRIVER_APK" ]; then
  echo ""
  ...
  RELEASE_RESP=$(curl -s -X POST -H "Authorization: token $GH_TOKEN" \
    ...
    -d "{\"tag_name\":\"$VERSION\",\"name\":\"JAGO Platform $VERSION — Blue+White Branding + Rapido Polish\",\"body\":\"## JAGO Platform $VERSION\\n\\n### Branding Overhaul\\n- ALL orange (#FF6B35) replaced with JAGO Blue (#1E6DE5) across both apps\\n- JAGO branding: Blue + White ONLY\\n- Customer Android native splash: dark navy (#060D1E) instead of orange\\n- Driver Android native splash: dark navy (unchanged)\\n- Both splash screens: blue glow + white logo\\n\\n### Theme Updates\\n- Light theme primary: #1E6DE5 (blue)\\n- Dark theme primary: #1E6DE5 (blue)\\n- Buttons, accents, gradients all updated to blue palette\\n- Razorpay checkout theme: blue\\n\\n### UI Polish\\n- Clean Rapido-style premium look\\n- Consistent blue+white color scheme\\n- All screens updated: home, booking, profile, wallet, onboarding\"}" \
  ...
  if [ -n "$RELEASE_ID" ]; then
    ...
    if [ -f "$CUSTOMER_APK" ]; then
      echo "Uploading Customer APK..."
      curl -s -X POST -H "Authorization: token $GH_TOKEN" \
        -H "Content-Type: application/vnd.android.package-archive" \
        --data-binary @"$CUSTOMER_APK" \
        "$UPLOAD_URL?name=JAGO-Customer-v1.0.26.apk" | grep -o '"state":"[^"]*"' | head -1
    fi
    ...
    if [ -f "$DRIVER_APK" ]; then
      ...
      curl -s -X POST -H "Authorization: token $GH_TOKEN" \
        -H "Content-Type: application/vnd.android.package-archive" \
        --data-binary @"$DRIVER_APK" \
        "$UPLOAD_URL?name=JAGO-Pilot-v1.0.26.apk" | grep -o '"state":"[^"]*"' | head -1
    fi
