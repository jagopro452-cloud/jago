...
import { getDriverEligibleServiceSnapshot } from "./dispatch-eligibility";
...
export async function getDriverEligibleServices(
  ...
  if (!snapshot.profile) return { services: [], parcelVehicles: [] };
  ...
  const eligibleParcelKeys: string[] = [...snapshot.parcelVehicleKeys];

  for (const svc of allServices) {
    if (!snapshot.serviceKeys.includes(svc.key)) {
      if (!(svc.category === 'parcel' && snapshot.profile.parcelEligibility && svc.key === 'parcel_delivery')) {
        continue;
      }
    }
    if (svc.category === 'parcel') {
