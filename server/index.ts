  ...
  (async () => {
    try {
      ...
      const settingsRes = await dbPool.query(
        "SELECT key_name, value FROM business_settings WHERE key_name IN ($1,$2,$3,$4,$5)",
        ["razorpay_key_id","razorpay_key_secret","razorpay_webhook_secret","google_maps_key","anthropic_api_key"]
      );
