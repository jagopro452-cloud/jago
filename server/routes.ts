...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/driver-wallet/:id/verify-payment", async (req, res) => {
    try {
      ...
      const row: any = updated.rows[0];
      const newBalance = parseFloat(row.wallet_balance);
      // Auto-unlock if balance is now above the auto-lock threshold
  ...
  app.patch("/api/app/driver/online-status", authApp, async (req, res) => {
    try {
      ...
      if (isOnline) {
        ...
        // If subscription model, check active subscription
        if (modelRow?.revenue_model === 'subscription') {
  ...
  app.post("/api/app/driver/accept-trip", authApp, async (req, res) => {
    try {
      ...
      const driverBusy = await rawDb.execute(rawSql`SELECT id FROM trip_requests WHERE driver_id=${driver.id}::uuid AND current_status IN ('accepted','arrived','on_the_way')`);
  ...
  app.post("/api/app/driver/reject-trip", authApp, async (req, res) => {
    try {
      ...
      if (tripRes.rows.length && io) {
        ...
        const rejectedIds = (trip.rejectedDriverIds || []).map((id: string) => `'${id}'::uuid`).join(',');
        const excludeClause = rejectedIds.length ? `AND u.id NOT IN (${rejectedIds})` : '';
  ...
  app.post("/api/app/driver/complete-trip", authApp, async (req, res) => {
    try {
      ...
      const tripInfo = await rawDb.execute(rawSql`SELECT estimated_fare, estimated_distance, current_status, payment_method, customer_id, trip_type, type FROM trip_requests WHERE id=${tripId}::uuid AND driver_id=${driver.id}::uuid`);
      if (!tripInfo.rows.length) return res.status(404).json({ message: "Trip not found" });
      const tripRow = tripInfo.rows[0] as any;
  ...
  app.post("/api/app/voice-booking/parse", authApp, async (req, res) => {
    try {
      ...
      const apiKey = process.env.GOOGLE_MAPS_API_KEY || 'AIzaSyB_yncy2ojljQ_dehITVkPQrPDtoCQbuhw';
  ...
  app.patch("/api/app/driver/update-registration", authApp, async (req, res) => {
    try {
      const user = (req as any).currentUser;
      const { dateOfBirth, city, vehicleBrand, vehicleColor, vehicleYear, licenseNumber, licenseExpiry, vehicleNumber, vehicleModel, vehicleType, selfieImage } = req.body;
      await rawDb.execute(rawSql`
        UPDATE users SET
  ...
  // ── DRIVER: Choose revenue model (commission or subscription) ──────────────
  app.post("/api/app/driver/choose-model", authApp, async (req, res) => {
    try {
      const driver = (req as any).currentUser;
      const { model } = req.body; // 'commission' | 'subscription'
      if (!['commission', 'subscription'].includes(model)) return res.status(400).json({ message: "Invalid model" });
      await rawDb.execute(rawSql`
  ...
  app.post("/api/app/driver/activate-subscription", authApp, async (req, res) => {
    try {
      ...
      // Mark model as subscription
      await rawDb.execute(rawSql`UPDATE users SET revenue_model='subscription', model_selected_at=NOW() WHERE id=${driver.id}::uuid`);
  ...
  app.post("/api/admin/cleanup-stale-trips", async (req, res) => {
    try {
  ...
  app.patch("/api/admin/drivers/:id/doc-review", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.patch("/api/admin/drivers/:id/verify-driver", async (req, res) => {
    try {
      const { id } = req.params;
    ...
    try {
      const r = await rawDb.execute(rawSql`
        SELECT cs.*,
          ...
          vc.name as vehicle_name,
          (cs.max_seats - COALESCE((SELECT COUNT(*) FROM car_sharing_bookings b WHERE b.ride_id = cs.id AND b.status != 'cancelled'),0)) as available_seats
        FROM car_sharing_rides cs
        LEFT JOIN users u ON u.id = cs.driver_id
        LEFT JOIN vehicle_categories vc ON vc.id = cs.vehicle_category_id
  ...
  app.post('/api/app/customer/car-sharing/book', authApp, async (req, res) => {
    try {
      ...
      const rideRes = await rawDb.execute(rawSql`
        SELECT cs.*, COALESCE((SELECT COUNT(*) FROM car_sharing_bookings b WHERE b.ride_id = cs.id AND b.status != 'cancelled'),0) as booked_count
        FROM car_sharing_rides cs WHERE cs.id = ${rideId}::uuid
      `);
      if (!rideRes.rows.length) return res.status(404).json({ message: 'Ride not found' });
      const ride = camelize(rideRes.rows[0]);
      const available = ride.maxSeats - parseInt(ride.bookedCount);
  ...
  setInterval(async () => {
    try {
      ...
      const timedOut = await rawDb.execute(rawSql`
        SELECT t.id, t.pickup_lat, t.pickup_lng, t.vehicle_category_id, t.driver_id,
      ...
      for (const row of timedOut.rows) {
        ...
        const rejectedIds = [...(trip.rejectedDriverIds || []), trip.driverId]
          .map((id: string) => `'${id}'::uuid`).join(',');
        ...
        if (nextBest.length && io) {
          ...
          notifyNearbyDriversNewTrip(trip.id, trip.pickupLat, trip.pickupLng, trip.vehicleCategoryId).catch(() => {});
        }
      }
    } catch (e: any) {
      console.error("[TIMEOUT] Auto-reassign error:", e.message);
    }
  }, 30000);
